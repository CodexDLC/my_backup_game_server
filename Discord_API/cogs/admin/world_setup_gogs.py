...
world_setup_logger = bot_logger.getChild(__name__)

# Импортируем наш оркестратор сервисов Discord сущностей
from Discord_API.core.services.admin import discord_entity_service
from Discord_API.core.services.admin.discord_entity_service import DiscordEntityService

# Импортируем BOT_PREFIX из настроек
from Discord_API.discord_settings import BOT_PREFIX
...
class WorldSetupCommands(commands.Cog):
    ...
    async def sync_roles_command(self, ctx: commands.Context):
        ...
        try:
            ...
            created_count = details.get("created_discord_roles", 0)
            updated_count = details.get("updated_discord_roles", 0)
            synced_count = details.get("synced_to_backend_count", 0)
            ...
            formatted_response += f"**Создано в Discord:** {created_count}\n"
            formatted_response += f"**Обновлено в Discord:** {updated_count}\n"
