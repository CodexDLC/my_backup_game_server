# game_server/app_discord_bot/app/services/navigation/navigation_orchestrator.py
...
class NavigationOrchestrator:
    ...
    async def process(self, command_str: str, interaction: discord.Interaction, response_message_object: discord.Message | None = None):
        ...
        if not LogicHandlerClass:
            self.logger.warning(f"Логический обработчик для команды '{command_name}' не найден в NavigationOrchestrator.")
            return
        ...
        if not PresentationHandlerClass:
            self.logger.warning(f"Презентационный обработчик для типа '{data_type}' не найден в NavigationOrchestrator.")
            return
