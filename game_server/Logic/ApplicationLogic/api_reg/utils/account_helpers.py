# game_server/ApplicationLogic/api_reg/utils/account_helpers.py
import secrets
import re
...
async def generate_linking_token(length: int = 64) -> str:
    """Генерирует криптографически стойкий шестнадцатеричный токен для одноразовой ссылки привязки."""
    return secrets.token_hex(length // 2) # length // 2, т.к. token_hex возвращает 2 символа на байт
